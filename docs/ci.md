...
The CI runs on every push to `main` and every pull request. It uses smart scoping to skip expensive jobs when only docs or native code changed. Forks use GitHub-hosted runners so the default pipeline can execute without external runner dependencies.
